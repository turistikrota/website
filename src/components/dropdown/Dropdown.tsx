...
type DropdownType = React.FC<React.PropsWithChildren> & {
  ...
  OverlayItem: React.FC<
    React.PropsWithChildren<PropsWithActive<OverlayItemProps>>
  >;
};
...
const Button: React.FC<React.PropsWithChildren<PropsWithActive>> = ({
  active,
  children,
}) => {
...
const Overlay: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  return (
    <div
      className={`
    origin-top-right absolute mt-2 right-0 w-40 rounded-md shadow-lg bg-white dark:bg-third p-1 ring-1 ring-black ring-opacity-5 transition-opacity
