...
const scaleValueForIndex = (index: number) =>
  Math.min(Math.max(1 + (2 / 15) * index - (1 / 15) * index * index, 0), 1);

const DotRenderer: React.FC<Props> = ({ count, indexes, onClick }) => {
  const [visibleRange, setVisibleRange] = useState<number[]>([]);

  useEffect(() => {
    const [fromIndex, currentIndex, toIndex] = indexes;
  ...
  const visibleWidth =
    visibleRange.reduce(
      (previous, index) =>
        previous +
        DOT_DIAMETER * scaleValueForIndex(index - indexes[0]) +
        DOT_DIAMETER,
      0
    ) +
      ...
      {Array.from({ length: count }, (_, index) => {
        ...
        const translateX =
          Array.from(
            { length: index },
            (_, i) =>
              DOT_DIAMETER * scaleValueForIndex(i - indexes[0]) + DOT_DIAMETER
